...
void CContUniformGravityLoad::Draw( void )
{
  PenState     pnState;
  Rect      myRect;
...
void CContUniformGravityLoad::HandleArrows()
{
  float    bSlope,tSlope;
...
void CContUniformGravityLoad::DoYourDialog( void )
{
  ...
  DHStandardNewRecord (&dhp, ContUnifGravLoadDLOG_ID,
              ContUnifGravLoadUpdateUserItemId,
              ContUnifGravLoad1stMagnitude);
  ...
  DHAdvAddEditReal(dhp, ContUnifGravLoad1stMagnitude,
  ...
  dForm.digits = 2;
  dForm.style = FIXEDDECIMAL;
  DHAdvAddEditReal(dhp, ContUnifGravLoadLength,
  ...
  if ( IamConnected && itsTElement)
  {
    dForm.digits = 2;
    dForm.style = FIXEDDECIMAL;
    DHAdvAddEditReal(dhp, ContUnifGravLoadTEConnectionRatio,
      (TElementConnectionRatio ),
        dForm, 0.0, 1.0);

    DHADDCHECKBOX( dhp, ContUnifGravLoadAlongStrutE, FALSE);
  }
  ...
  {
    new1stEndLoad = DHGetEditReal(dhp, ContUnifGravLoad1stMagnitude);
    newContLoadLength = DHGetEditReal(dhp, ContUnifGravLoadLength);
    if( IamConnected )
      newTElementConnectionRatio =
          DHGetEditReal(dhp, ContUnifGravLoadTEConnectionRatio);
...
void CContUniformGravityLoad::MoveYourself( void )
{
  Point    temp1, temp2;
...
void  CContUniformGravityLoad::FromtheHitPointToCorners( void )
{
  int    verticalOffset;

  if ( topLeftCornerFlag )
  {
    verticalOffset = topRightCorner.v - topLeftCorner.v;
    topLeftCorner.v = theHitPoint.v - offSet.v;
    topRightCorner.v = topLeftCorner.v + verticalOffset;
    if( ! IamConnected )
    {
  ...
  else if ( topRightCornerFlag )
  {
    ...
    topLeftCorner.v = topRightCorner.v - verticalOffset;
    if( ! IamConnected )
    {
...
void  CContUniformGravityLoad::FromCornersToItsCenterPoint( void )
{
  itsCenterPoint.v = (( bottomLeftCorner.v + topLeftCorner.v ) / 2 +
            ( bottomRightCorner.v + topRightCorner.v ) / 2)/2;
  itsCenterPoint.h = (( bottomLeftCorner.h + topLeftCorner.h ) / 2 +
