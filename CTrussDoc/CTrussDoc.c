...
void CTrussDoc::ITrussDoc(CBureaucrat *aSupervisor, Boolean printable)

{
  ...
  horzDrawingSize = 50 * 12; /*/ theStructureScale * SCREEN_DPI;/* 50 ft.*/
  vertDrawingSize= 25 * 12; /*/ theStructureScale * SCREEN_DPI; /* 25 ft.*/

}

void CTrussDoc::DrawingSizeDialog( void )
{
...
void CTrussDoc::DoCommand(long theCommand)
...
void CTrussDoc::Deactivate(void)
...

{
  ...
  sturctureNumber = ((CTrussApp *)gApplication)->structureNumber;

  wCount = gDecorator->GetWCount();  /*   default name of the window    */
  NumToString((long)sturctureNumber, wNumber);
  ConcatPStrings(wTitle, (StringPtr) "\pStructure-");
...
void CTrussDoc::OpenFile(SFReply *macSFReply)
...
void CTrussDoc::BuildWindow (Handle theData)

{
  CScrollPane    *theScrollPane;
  CTrussPane    *theMainPane;
  int        paneWidth;
  int        paneHeight;
  ...
  itsWindow->IWindow(WINDTruss, FALSE, gDesktop, this);
  ...
  theMainPane->ITrussPane(theScrollPane, this, paneWidth, paneHeight, 0, 0, sizELASTIC, sizELASTIC);
  theMainPane->FitToEnclosure(TRUE, TRUE);
     ...
     ** of the following variables, one in CTrussDoc, and the other is in
     ** CTrussPane
     **/
...
Boolean CTrussDoc::DoSaveAs(SFReply *macSFReply)
 ...
 **************************************************************************/
short  CTrussDoc::PageCount( void )
{
  long  pixWidth;
  long  pixHeight;
  long  height;
